module EventCalendar
  ...
  module ClassMethods
    ...
    def events_for_date_range(start_d, end_d, find_options = {})
      self.where(find_options).where(
        "(? <= #{self.end_at_field}) AND (#{self.start_at_field}< ?)", start_d.to_time.utc, end_d.to_time.utc
  ...
  module InstanceMethods
    ...
    def clip_range(start_d, end_d)
      ...
      unless tour_weekends?
        clipped_start += 1 while [0,6].include?(clipped_start.wday)
        clipped_end -= 1 while [0,6].include?(clipped_end.wday)
      end
